...
// 🛫 ২. ৩×৩ মাল্টি-পেলাইন স্লট কোর এপিআই রাউট (POST Route - ৯৫% RTP গাণিতিক বর্ম কঠোর লক ভাই ভাই!)
app.post('/api/rainbow-spin', async (req, res) => {
    ...
    try {
        ...
        // 🔒 [কровать লক বর্ম]: পকেটে বাজি ধরার চেয়ে কম টাকা থাকলে বাজি ডিরেক্ট রিফিউজড ভাই ভাই!
        ...
        while (isLoopActive && loopSafety < 200) {
            ...
            // 🔍 नियम ১: ৫টি স্ট্যান্ডার্ড পেলাইন চেক (পাশাপাশি ও কোনাকুনি)
            ...
            // 🔍 नियम ২: এনিহোয়ার ৩-ম্যাচ স্ক্যাটার চেক (যেকোনো জায়গায় ৩টি একই ফল থাকলে উইন ভাই ভাই!)
            const symbolCounts = {};
            const symbolCoords = {};
            ...
            // 🎯 [🎰 ৯৫% ওরিজিনাল ক্যাসিনো RTP কঠোর নিয়ন্ত্রণ বর্ম ভাই ভাই]
            if (adminTriggeredPrize) {
                ...
                if (finalStatus === "win") {
                    // মাল্টি-পেলাইন উইন রেশিও ও গ্লোবাল ক্যাসিনো আরটিপি লুপ নিখুঁত ৯৫% এ টাইট লক ভাই ভাই
                    if (Math.random() <= 0.35) {
                        isLoopActive = false;
                    }
                } else {
